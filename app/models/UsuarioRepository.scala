...
case class Enlace(enla_id: Option[Long], enla_uuid: Option[String], enla_activo: Option[Int], enla_fecha: Option[DateTime], usua_email: Option[String])
...
class UsuarioRepository @Inject()(dbapi: DBApi)(implicit ec: DatabaseExecutionContext) {
    ...
    */
    def buscarPorEmail(email: String): Future[Option[Usuario]] = Future {
      db.withConnection { implicit connection => 
        var result = SQL("SELECT * FROM \"gen$empleado\" WHERE email = {email}").on('email -> email).as(Usuario.usuarioRS.singleOpt)
        println("Usuario: " + result)
        result
      }
    }(ec)  

    def autenticar(email: String, contrasena: String): Future[Boolean] = Future {
        db.withConnection { implicit connection =>
            SQL("UPDATE \"gen$empleado\" SET ultima_sesion = CURRENT_TIMESTAMP WHERE email = {email} AND contrasena = {contrasena} AND tipo <> 99").
            on(
                "email" -> email, 
                "contrasena" -> Conversion.sha1(contrasena)).executeUpdate() > 0
        }
    }(ec)
    ...
    */
    ...
    def cambiarClave(link: String, clave: String): Boolean = {
        ...
        db.withConnection { implicit connection => 
            ...
            val result:Boolean = SQL("UPDATE \"gen$empleado\" SET contrasena = {contrasena} WHERE email = {email}").
            on(
                'email -> email,
    ...
    def crear(usuario: Usuario): Future[Long] = Future {
        db.withConnection { implicit connection =>
            val fecha: LocalDate = new LocalDate(Calendar.getInstance().getTimeInMillis())
            val hora: LocalDateTime = new LocalDateTime(Calendar.getInstance().getTimeInMillis())
            val id: Long = SQL("INSERT INTO \"gen$empleado\" (id_empleado, documento, primer_apellido, segundo_apellido, nombre, numero_cuenta, ultimo_cambio_pasabordo, ag, contrasena, tipo, ultima_sesion, email, empr_id, perf_id, id) VALUES({id_empleado}, {documento}, {primer_apellido}, {segundo_apellido}, {nombre}, {numero_cuenta}, {ultimo_cambio_pasabordo}, {ag}, {contrasena}, {tipo}, {ultima_sesion}, {email}, {empr_id}, {perf_id}, {id}) RETURNING id").
            on(
                'id_empleado -> usuario.id_empleado,
                'documento -> usuario.documento,
                'primer_apellido -> usuario.primer_apellido, 
            ...
            SQL("INSERT INTO usuario_empresa (id_empleado, empr_id, perf_id) VALUES ({id_empleado}, {empr_id}, {perf_id})").
            on(
                'id_empleado -> id,
                'empr_id -> usuario.empr_id,
                'perf_id -> usuario.perf_id
    ...
    def actualizar(id_empleado: String, contrasena: String): Future[Boolean] = Future {
        db.withConnection { implicit connection =>
            SQL("UPDATE \"gen$empleado\" SET ultima_sesion = now() WHERE id_empleado = {id_empleado} AND contrasena = {contrasena})").
            on(
    ...
    def actualizar(usuario: Usuario, email: String) : Future[Boolean] = {
        ...
        db.withConnection { implicit connection => 
            val fecha: LocalDate = new LocalDate(Calendar.getInstance().getTimeInMillis())
            val hora: LocalDateTime = new LocalDateTime(Calendar.getInstance().getTimeInMillis())
            val count: Long = SQL("UPDATE \"gen$empleado\" SET contrasena = {contrasena}, nombre = {nombre}, primer_apellido = {primer_apellido}, segundo_apellido = {segundo_apellido}, tipo = {tipo}, ultima_sesion = {ultima_sesion} WHERE email = {email}").
            on(
  ...
  */
  def cuenta(empr_id: Long): Long =  {
    db.withConnection{ implicit connection =>
      val result = SQL("SELECT COUNT(*) AS c FROM \"gen$empleado\" u WHERE empr_id = {empr_id}").
      on(
          'empr_id -> empr_id
      ).as(SqlParser.scalar[Long].single)
      result
    ...
    def todos(empr_id: Long, page_size:Long, current_page:Long): Future[Iterable[Usuario]] = Future[Iterable[Usuario]] {
                db.withConnection { implicit connection => 
                    SQL("SELECT FIRST {page_size} SKIP ({page_size} * ({current_page} - 1)) * FROM \"gen$empleado\" u WHERE empr_id = {empr_id} ORDER BY primer_apellido").
                    on(
                       'empr_id -> empr_id,
                       'page_size -> page_size,
