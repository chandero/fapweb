...
object Persona_b {
  ...
  implicit val pbReads: Reads[Persona_b] = (
    (__ \ "id_apoderado").readNullable[String] and
      ...
      (__ \ "id_ciiu").readNullable[String] and
...
class PersonaRepository @Inject()(dbapi: DBApi)(
  ...
  def obtenerDirecto(
    ...
    def obtenerPersonaPorColocacion(id_colocacion: String): Future[Option[Persona]] = Future {
      db.withConnection { implicit connection =>
        val persona_a = SQL(
          """SELECT p.* FROM "col$colocacion" c 
