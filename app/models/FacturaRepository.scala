...
import anorm._
import anorm.SqlParser.{get, str, date, int, long, double}
...
case class _CompradorFactura(
    ...
    CompradorIdentProcedencia: Option[String]
)
...
case class _LsDetalle(
    ...
    FechaCompra: Option[String],
    CodDescripcionGenTra: Option[String],
    DescripcionGenTra: Option[String]
)
...
case class _LsDetalleImpuesto(
    ...
    EsRetencionImpuesto: Option[String],
...
/* object _LsImpuestos {
  ...
  implicit val Reads: Reads[_LsImpuestos] = (
    ...
    (__ \ "EsRetencionImpuesto").readNullable[String] and
  ...
  val _set = {
    get[Option[String]]("BaseImponible") ~
    get[Option[String]]("CodigoImpuesto") ~
    get[Option[String]]("EsRetencionImpuesto") ~
    get[Option[String]]("NombreImpuesto") ~
    get[Option[String]]("Porcentaje") ~
    get[Option[String]]("ValorImpuesto") map {
      case a ~ b ~ c ~ d ~ e ~ f => _LsImpuestos(a, b, c, d, e, f)
    }
  }
} */
/*
...
object FacturaItem {
  ...
  implicit val Reads: Reads[FacturaItem] = (
    (__ \ "fait_id").readNullable[Long] and
      ...
      (__ \ "fait_total").readNullable[Double] and
      (__ \ "fait_codigo").readNullable[String]
  )(FacturaItem.apply _)
  ...
  val _set = {
    get[Option[Long]]("fait_id") ~
      ...
      get[Option[Double]]("fait_total") ~
      get[Option[String]]("fait_codigo") map {
      case fait_id ~
            fact_numero ~
            fait_detalle ~
            fait_cantidad ~
...
class FacturaRepository @Inject()(
  ...
  /**
    ...
    @param fact_numero: Long
    */
  def buscarDSAPorNumero(fact_numero: Long): Future[Option[Factura]] =
    Future[Option[Factura]] {
      db.withConnection { implicit connection =>
        val f = SQL(
          """SELECT  f1.DSA_NUMERO AS FACT_NUMERO, 
              ...
              f1.ID_PERSONA,
              f1.ID_EMPLEADO,
              f1.DSA_ESTADO AS FACT_ESTADO, gp1.PRIMER_APELLIDO, gp1.SEGUNDO_APELLIDO, gp1.NOMBRE, (SELECT SUM(DSIT_TOTAL) FROM DSA_ITEM fi1 WHERE fi1.DSA_NUMERO = f1.DSA_NUMERO) AS FACT_TOTAL FROM DSA f1
                     LEFT JOIN "gen$persona" gp1 ON gp1.ID_IDENTIFICACION = f1.ID_IDENTIFICACION AND gp1.ID_PERSONA = f1.ID_PERSONA
                     WHERE f1.DSA_ESTADO <> 9 AND f1.DSA_NUMERO = {fact_numero}"""
        ).on(
            'fact_numero -> fact_numero
          )
          .as(Factura._set.singleOpt)
          ...
          case Some(f) =>
            val fis = SQL(
              """SELECT 
                  ...
                  DSIT_DETALLE AS FAIT_DETALLE,
                  DSIT_CANTIDAD AS FAIT_CANTIDAD,
                  DSIT_VALORUNITARIO AS FAIT_VALORUNITARIO,
                  DSIT_TASAIVA AS FAIT_TASAIVA,
                  DSIT_VALORIVA AS FAIT_VALORIVA,
                  DSIT_TOTAL AS FAIT_TOTAL,
                  DSIT_CODIGO AS FAIT_CODIGO 
                 FROM DSA_ITEM WHERE DSA_NUMERO = {fact_numero}"""
            ).on('fact_numero -> f.fact_numero).as(FacturaItem._set *)
            var factura = f.copy(items = Some(fis))
            Some(factura)
          case None => None
  ...
  def cuentaDSA(): Long = {
    db.withConnection { implicit connection =>
      SQL("""SELECT COUNT(*) FROM DSA f1 WHERE f1.DSA_ESTADO <> 9""").as(
        SqlParser.scalar[Long].single
      )
    }
  }
  ...
  def todosDSA(
    ...
    db.withConnection { implicit connection =>
      var query =
        """SELECT FIRST {page_size} SKIP ({page_size} * ({current_page} - 1))  
              ...
              gp1.NOMBRE, 
              (SELECT SUM(DSIT_TOTAL) FROM DSA_ITEM fi1 WHERE fi1.DSA_NUMERO = f1.DSA_NUMERO) AS FACT_TOTAL FROM DSA f1
                     LEFT JOIN "gen$persona" gp1 ON gp1.ID_IDENTIFICACION = f1.ID_IDENTIFICACION AND gp1.ID_PERSONA = f1.ID_PERSONA
                     WHERE f1.DSA_ESTADO <> 9"""
      if (!filter.isEmpty) {
        query = query + " and " + filter
      }
      if (!orderby.isEmpty) {
        ...
        query = query + s" ORDER BY DSA_NUMERO DESC"
      }
  ...
  def todosNotaDebito(
    ...
    db.withConnection { implicit connection =>
      var query =
        """SELECT FIRST {page_size} SKIP ({page_size} * ({current_page} - 1)) 
            fn1.FACT_NOTA_TIPO, fn1.FACT_NOTA_NUMERO, fn1.FACT_NOTA_FECHA, fn1.FACT_NOTA_DESCRIPCION, fn1.FACT_NUMERO AS FACT_NUMERO, fn1.ID_EMPLEADO, fn1.FACT_NOTA_ESTADO, fn1.FACT_CUFE, fn1.FACT_NOTA_CONCEPTO_NUMERO,
                  gp1.ID_IDENTIFICACION, gp1.ID_PERSONA, gp1.PRIMER_APELLIDO, gp1.SEGUNDO_APELLIDO, gp1.NOMBRE, (SELECT SUM(FANOIT_TOTAL) FROM FACTURA_NOTA_ITEM fni1 WHERE fni1.FACT_NOTA_TIPO = 'D' AND fni1.FACT_NOTA_NUMERO = fn1.FACT_NOTA_NUMERO) AS FACT_NOTA_TOTAL FROM FACTURA_NOTA fn1
  ...
  def todosNotaCredito(
    ...
    db.withConnection { implicit connection =>
      var query =
        """SELECT FIRST {page_size} SKIP ({page_size} * ({current_page} - 1)) 
              fn1.FACT_NOTA_TIPO, fn1.FACT_NOTA_NUMERO, fn1.FACT_NOTA_FECHA, fn1.FACT_NOTA_DESCRIPCION, fn1.FACT_NUMERO, fn1.ID_EMPLEADO, fn1.FACT_NOTA_ESTADO, fn1.FACT_CUFE, fn1.FACT_NOTA_CONCEPTO_NUMERO,
              gp1.ID_IDENTIFICACION, gp1.ID_PERSONA, gp1.PRIMER_APELLIDO, gp1.SEGUNDO_APELLIDO, gp1.NOMBRE, (SELECT SUM(FANOIT_TOTAL) FROM FACTURA_NOTA_ITEM fni1 WHERE fni1.FACT_NOTA_TIPO = 'C' AND fni1.FACT_NOTA_NUMERO = fn1.FACT_NOTA_NUMERO) AS FACT_NOTA_TOTAL FROM FACTURA_NOTA fn1
                     LEFT JOIN FACTURA f1 ON f1.FACT_NUMERO = fn1.FACT_NUMERO
                     LEFT JOIN "gen$persona" gp1 ON gp1.ID_IDENTIFICACION = f1.ID_IDENTIFICACION AND gp1.ID_PERSONA = f1.ID_PERSONA
  ...
  //
  // NOTA CREDITO
  def cuentaNotaCreditoDSA(): Long = {
    db.withConnection { implicit connection =>
      SQL(
        """SELECT COUNT(*) FROM FACTURA_NOTA fn1 WHERE fn1.FACT_NOTA_TIPO = 'S' AND fn1.FACT_NOTA_ESTADO <> 9"""
      ).as(
        SqlParser.scalar[Long].single
      )
    }
  }  
  def todosNotaCreditoDSA(
      page_size: scala.Long,
      current_page: scala.Long,
      empr_id: scala.Long,
      orderby: String,
    ...
    db.withConnection { implicit connection =>
      var query =
        """SELECT FIRST {page_size} SKIP ({page_size} * ({current_page} - 1)) 
              fn1.FACT_NOTA_TIPO, fn1.FACT_NOTA_NUMERO, fn1.FACT_NOTA_FECHA, fn1.FACT_NOTA_DESCRIPCION, fn1.FACT_NUMERO, fn1.ID_EMPLEADO, fn1.FACT_NOTA_ESTADO, fn1.FACT_CUFE, fn1.FACT_NOTA_CONCEPTO_NUMERO,
              gp1.ID_IDENTIFICACION, gp1.ID_PERSONA, gp1.PRIMER_APELLIDO, gp1.SEGUNDO_APELLIDO, gp1.NOMBRE, (SELECT SUM(FANOIT_TOTAL) FROM FACTURA_NOTA_ITEM fni1 WHERE fni1.FACT_NOTA_TIPO = 'S' AND fni1.FACT_NOTA_NUMERO = fn1.FACT_NOTA_NUMERO) AS FACT_NOTA_TOTAL FROM FACTURA_NOTA fn1
                     LEFT JOIN DSA f1 ON f1.DSA_NUMERO = fn1.FACT_NUMERO
                     LEFT JOIN "gen$persona" gp1 ON gp1.ID_IDENTIFICACION = f1.ID_IDENTIFICACION AND gp1.ID_PERSONA = f1.ID_PERSONA
                     WHERE fn1.FACT_NOTA_ESTADO <> 9 AND fn1.FACT_NOTA_TIPO = 'S'"""
      if (!filter.isEmpty) {
        query = query + " AND " + filter
      }
      if (!orderby.isEmpty) {
      ...
      for (nota <- notas) {
        val items = SQL(
          """SELECT * FROM FACTURA_NOTA_ITEM fni1 WHERE fni1.FACT_NOTA_TIPO = 'S' AND fni1.FACT_NOTA_NUMERO = {fact_nota_numero}"""
        ).on(
            'fact_nota_numero -> nota.fact_nota_numero
          )
          .as(FacturaNotaItem._set *)
  ...
  /**
    Recuperar una Nota Credito DSA usando su DSA_NUMERO
    @param fact_nota_numero: Long
    */
  def buscarNotaCreditoDSAPorNumeroDirecto(fact_nota_numero: Long): FacturaNota = {
    db.withConnection { implicit connection =>
      println("Buscando Nota Credito DSA No.:" + fact_nota_numero)
      val nc = SQL(
        """SELECT fn1.*, gp1.ID_IDENTIFICACION, gp1.ID_PERSONA, gp1.PRIMER_APELLIDO, gp1.SEGUNDO_APELLIDO, gp1.NOMBRE, (SELECT SUM(FANOIT_TOTAL) FROM FACTURA_NOTA_ITEM fni1 WHERE fni1.FACT_NOTA_NUMERO = fn1.FACT_NOTA_NUMERO) AS FACT_NOTA_TOTAL FROM FACTURA_NOTA fn1
                     LEFT JOIN DSA f1 ON f1.DSA_NUMERO = fn1.FACT_NUMERO
                     LEFT JOIN "gen$persona" gp1 ON gp1.ID_IDENTIFICACION = f1.ID_IDENTIFICACION AND gp1.ID_PERSONA = f1.ID_PERSONA
                     WHERE fn1.FACT_NOTA_ESTADO <> 9 AND fn1.FACT_NOTA_TIPO = 'S' AND fn1.FACT_NOTA_NUMERO = {fact_nota_numero}"""
      ).on(
          'fact_nota_numero -> fact_nota_numero
        )
        .as(FacturaNota._set.singleOpt)
      ...
      nc match {
        case Some(nc) =>
          println("Buscando Items")
          val fnis = SQL(
            "SELECT * FROM FACTURA_NOTA_ITEM WHERE FACT_NOTA_TIPO = 'S' AND FACT_NOTA_NUMERO = {fact_nota_numero}"
          ).on('fact_nota_numero -> nc.fact_nota_numero)
            .as(FacturaNotaItem._set *)
          var nota = nc.copy(items = Some(fnis))
          nota
  ...
  def enviarFactura(fact_numero: Long): Future[_RootInterface] =
    Future[_RootInterface] {
      db.withConnection { implicit connection =>
        ...
        ).as(SqlParser.str("DPTO").singleOpt)
        ...
        val prefijo: String = autorizacion._5 match {
          case Some(p) => p
          case None    => ""
        }
        ...
        val _compradorData = new _CompradorFactura(
          ...
          tipoiden match {
            case 42 => depa_nombre
            case _ => Some(departamento.get._2)
          },          
          direccion.direccion,
          Some(_sendemail),
          Some(_ident),
  ...
  def crearNota(nota: FacturaNota, usua_id: Long): Future[Long] = {
    ...
    db.withConnection { implicit connection =>
      ...
      val fact_nota_numero = nota.fact_nota_tipo match {
        case Some(t) =>
          t match {
            ...
            case "S" => 
              SQL(
                "SELECT GEN_ID(GEN_FACTURA_NOTA_DSA_NUMERO, 1) FROM RDB$DATABASE;"
              ).as(SqlParser.scalar[Long].single)
          }
        case None => -1
      }
      ...
      Future.successful(empleado match {
        case Some(user) =>
          // Crear Encabezado Nota
          print("Guardando Nota Header :" + nota.fact_numero)
          val cufe = nota.fact_cufe match { case Some(f) => f case None => ""}
          val creado = SQL(queryNota)
            .on(
              ...
              'FACT_NOTA_ESTADO -> 1,
              'ID_EMPLEADO -> user.id_empleado,
              'FACT_NOTA_CONCEPTO_NUMERO -> 1
            )
            .executeInsert()
            .get > 0
          print("Guardar Nota Items :" + nota.fact_numero)
          if (creado) {
            print("Guardando Nota Items :" + nota.fact_numero)
            nota.items match {
              case Some(items) =>
                items.map {
                  item =>
    ...
    Future[_RootInterface] {
      db.withConnection { implicit connection =>
        ...
        val _compradorData = new _CompradorFactura(
          ...
          persona.a.get.pais_nacimiento.get match {
            case "COLOMBIA" => Some("Colombia")
            case "EE.UU."   => Some("ESTADOS UNIDOS DE AMERICA")
            case "EE.UU"    => Some("ESTADOS UNIDOS DE AMERICA")
            case _          => Some("Colombia")
          }, 
          Some(f.id_comprobante.toString()),
  ...
  // FIN NOTA CREDITO
  def enviarNotaCreditoDSA(fact_nota_numero: Long): Future[_RootInterface] =
    Future[_RootInterface] {
      db.withConnection { implicit connection =>
        var _rootInterface = new _RootInterface(
          None,
        ...
        val nc = buscarNotaCreditoDSAPorNumeroDirecto(fact_nota_numero) /* SQL("""SELECT * FROM FACTURA f
                            WHERE f.FACT_NUMERO = {fact_numero}""").on('fact_numero -> fact_numero).as(Factura._set.singleOpt) */
        val f = buscarDSAPorNumeroDirecto(nc.fact_numero.get)
        println("Nota Crédito DSA: " + nc)
        val persona = personaService.obtenerDirecto(
          nc.id_identificacion.get,
          nc.id_persona.get
        )
        ...
        println("cod_municipio: " + cod_municipio)
        println("depa_id :" + depa_id)
        val _parseAutorizacion = int("DSAU_ID") ~
          date("DSAU_FECHAFINAL") ~
          date("DSAU_FECHAINICIO") ~
          str("DSAU_NUMAUTORIZACION") ~
          str("DSAU_PREFIJO") ~
          int("DSAU_SECUENCIAFINAL") ~
          int("DSAU_SECUENCIAINICIAL") map {
          case a ~ b ~ c ~ d ~ e ~ f ~ g => (a, b, c, d, e, f, g)
        }
        println("Buscando Autorización para DSA No. " + nc.fact_numero)
        val autorizacion =
          SQL("""
                                SELECT * FROM DSA_AUTORIZACION 
                                WHERE {fact_numero} BETWEEN DSAU_SECUENCIAINICIAL AND DSAU_SECUENCIAFINAL
        ...
        val _compradorData = new _CompradorFactura(
          tipoiden match {
            case 42 => Some(persona.a.get.nombre.get.split(" ")(1))
            case 31 => Some(persona.a.get.nombre.get.split(" ")(1))
            case _  => persona.a.get.primer_apellido
          },
          direccion.municipio,
          tipoiden match {
            case 42 => Option.empty[String]
        ...
        val _softwareSeguridadData = new _SoftwareSeguridad(
          Some(_sSeguridad._1),
          Some("FPDSA" + nc.fact_nota_numero.get.toString),
          Some(_sSeguridad._2),
          Some(_sSeguridad._3),
          Some(_sSeguridad._4),
          Some("FPDSA" + nc.fact_nota_numero.get.toString),
  ...
  def enviarDSA(dsa_numero: Long): Future[_RootInterface] =
    Future[_RootInterface] {
      db.withConnection { implicit connection =>
        ...
        val _sSeguridad =
          SQL(
            "SELECT * FROM DSA_SOFTWARE_SEGURIDAD WHERE DSAU_ID = {dsau_id}"
          ).on(
        ...
        var nombre = persona.a.get.nombre
        var nombres = nombre.get.split(" ")
        var primer_nombre = ""
        var primer_apellido = ""
        ...
        val _compradorData = new _CompradorFactura(
          tipoiden match {
            case 42 => Some(persona.a.get.nombre.get.trim.split(" ")(1))
            case 31 => persona.a.get.nombre
            case _ => persona.a.get.primer_apellido
          },
          direccion.municipio,
        ...
        f.items.foreach { items =>
          items.foreach { item =>
            val _detalleData = new _LsDetalle(
              Some("1"),
              Some("999"),
              Some(item.fait_codigo.get),
              Some(item.fait_detalle.get.substring(0, {
                if (item.fait_detalle.get.length() > 44) {
                  44
                } else {
                  item.fait_detalle.get.length()
                }
              })),
              item.fait_detalle,
              Some(i),
        ...
        // Recorrer Impuestos
/*         val _parseImpuesto = double("DSIM_BASE") ~ str("DSIM_CODIGO") ~ int("DSIM_ESRETENCION") ~
            str("DSIM_NOMBRE") ~ double("DSIM_TASA") ~ double("DSIM_VALOR") map {
              case a ~ b ~ c ~ d ~ e ~ f => (a,b,c,d,e,f)
            }
        val _impuestos = SQL("""SELECT * FROM DSA_IMPUESTO WHERE DSA_NUMERO = {dsa_numero}""").
        on(
          'dsa_numero -> f.fact_numero
        ).as(_parseImpuesto *) */

        var _listImpuestoData = new ListBuffer[_LsImpuestos]()
/*         _impuestos.foreach { impuesto => 
          val _data = _LsImpuestos(
            Some(impuesto._1.toString()),
            Some(impuesto._2.toString()),
            Some((impuesto._3 match { case 1 => true case 0 => false })),
            Some(impuesto._4.toString()),
            Some(impuesto._5.toString()),
            Some(impuesto._6.toString())
         )
         _listImpuestoData += _data
        } */
        // 
