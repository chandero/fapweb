...
class AuxiliarRepository @Inject()(dbapi: DBApi, _gcon: GlobalesCon, _convert: Conversion, empresaService: EmpresaRepository)(
  ...
  def consultar(codigo_inicial: String, codigo_final: String, anho: Integer, fecha_inicial: Long, fecha_final: Long, id_identificacion: Int, id_persona: String): Iterable[Auxiliar] = {
    ...
    val queryPuc = """SELECT * FROM "con$puc" p WHERE p.CODIGO >= {codigo_inicial} AND p.CODIGO <= {codigo_final} AND p.MOVIMIENTO = 1 ORDER BY p.CODIGO ASC"""

    var queryMovs = """SELECT a1.*, ae1.*, ctc1.ABREVIATURA, cp1.NOMBRE AS CUENTA, TRIM(p1.NOMBRE || ' ' || p1.PRIMER_APELLIDO || ' ' || p1.SEGUNDO_APELLIDO) AS persona 
                  ...
                  LEFT JOIN "con$auxiliarext" ae1 ON ae1.ID = a1.ID
    ...
    db.withTransaction { implicit connection =>
      ...
      _listPuc.foreach { p => 
                  var _saldo_anterior = _gcon.saldo_anterior_codigo_puc(p.codigo.get, ffsi.getMillis())
