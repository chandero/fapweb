...
class AdicionalRepository @Inject()(dbapi: DBApi)(
  ...
  def activos(): Future[Iterable[Adicional]] = Future[Iterable[Adicional]] {
    val result = db.withConnection { implicit connection =>
      SQL("""SELECT * FROM \"col$descuentos\" WHERE ES_ACTIVO = 1""").as(
        Adicional._set *
      )
    }
    result
  ...
  def buscar(id_solicitud: String): Future[Iterable[SolicitudAdicional]] = {
    val result = db.withConnection { implicit connection =>
      SQL(
        """SELECT * FROM \"col$solicituddescuento\" WHERE ID_SOLICITUD = {id_solicitud}"""
      ).on(
