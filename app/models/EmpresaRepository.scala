...
class EmpresaRepository @Inject()(dbapi: DBApi)(
  ...
  private val simple = {
    get[Option[Long]]("empresa.empr_id") ~
      ...
      get[Long]("empresa.muni_id") map {
      case empr_id ~ empr_descripcion ~ empr_sigla ~ empr_identificacion ~ empr_direccion ~ empr_telefono ~ empr_estado ~ usua_id ~ depa_id ~ muni_id =>
        Empresa(empr_id,
                empr_descripcion,
                empr_sigla,
                empr_identificacion,
  ...
  def buscarPorId(empr_id: Long): Option[Empresa] = {
    db.withConnection { implicit connection =>
      SQL("SELECT * FROM empresa WHERE empr_id = {empr_id} and empr_estado <> 9")
        .on(
          'empr_id -> empr_id
        ).as(simple.singleOpt)
    }
  ...
  def buscarPorDescripcion(
    ...
    Future[Iterable[Empresa]] {
      db.withConnection { implicit connection =>
        SQL("SELECT * FROM siap.empresa WHERE empr_descripcion LIKE %{empr_descripcion}% ORDER BY empr_descripcion")
          .on(
  ...
  */
  ...
  def todos(page_size:Long, current_page:Long): Future[Iterable[Empresa]] = Future[Iterable[Empresa]] {
    db.withConnection { implicit connection =>
      SQL("SELECT * FROM siap.empresa WHERE empr_estado <> 9 ORDER BY empr_descripcion LIMIT {page_size} OFFSET {page_size} * ({current_page} - 1)").
      on(
        'page_size -> page_size,
  ...
  */
  def buscarPorUsuario(usua_id: Long): Future[Iterable[Empresa]] = Future[Iterable[Empresa]] {
    db.withConnection { implicit connection =>
      SQL("SELECT e.* FROM siap.usuario_empresa ue INNER JOIN siap.empresa e ON e.empr_id = ue.empr_id WHERE ue.usua_id = {usua_id} and empr_estado <> 9 ORDER BY e.empr_descripcion").
      on(
        'usua_id -> usua_id
      ).as(simple *)
  ...
  def actualizar(empresa: Empresa): Boolean = {
    ...
    db.withConnection { implicit connection =>
      ...
      val result: Boolean = SQL(
        "UPDATE siap.empresa SET empr_descripcion = {empr_descripcion}, empr_identificacion = {empr_identificacion}, empr_direccion = {empr_direccion}, empr_telefono = {empr_telefono}, empr_estado = {empr_estado}, usua_id = {usua_id}, depa_id = {depa_id}, muni_id = {muni_id} WHERE empr_id = {empr_id}")
        .on(
          'empr_id -> empresa.empr_id,
          'empr_descripcion -> empresa.empr_descripcion,
          'empr_identificacion -> empresa.empr_identificacion,
