...
class BalanceRepository @Inject()(
    dbapi: DBApi,
    _gcon: GlobalesCon,
    _convert: Conversion,
  ...
  def consultar(
    ...
    val querySaldo =
      """SELECT SUM(ca1.DEBITO) AS DEBITO, SUM(ca1.CREDITO) AS CREDITO FROM "con$comprobante" cc1
    ...
    db.withTransaction { implicit connection =>
      ...
      _resultSetPuc.map { puc =>
        ...
        var _credito_ant = 0d
        if (periodo > 1) {
          val _saldoParse = get[Option[Double]]("debito") ~ get[Option[Double]](
            "credito"
          ) map { case a ~ b => (a, b) }
          val _resultSetSaldo = SQL(querySaldo)
            .on(
              'fecha_inicial -> fisi,
              'fecha_final -> ffsi,
              'codigo -> (codigo + "%"),
              ...
              i =>
                if (columnas == 6) {
                  _listRow01 += com.norbitltd.spoiwo.model.Row(
                    StringCell(
                      i.codigo match {
                        case Some(d) => _convert.codigopuc(d)
                        case None    => ""
                      },
                      Some(0),
