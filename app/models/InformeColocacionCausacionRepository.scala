...
case class InformeColocacionCausacion(
...
case class Extracto(
    ...
    anticipado: Option[BigDecimal],
    cxc: Option[BigDecimal],
    servicio: Option[BigDecimal],
    fecha_interes: Option[DateTime],
    detallado: Option[List[Detallado]]
)
case class Detallado(
    ...
    dias: Option[Int],
    tasa: Option[BigDecimal],
    debito: Option[BigDecimal],
    credito: Option[BigDecimal]
)
case class Causado(
    fecha: Option[DateTime],
    valor: Option[BigDecimal],
    deuda: Option[BigDecimal],
    anticipado: Option[BigDecimal],
    causado: Option[BigDecimal],
    contingencia: Option[BigDecimal],
    dias: Option[Int],
    tasa: Option[BigDecimal],
    fecha_interes: Option[DateTime],
    extracto: Option[List[Extracto]]
)
...
class InformeColocacionCausacionRepository @Inject()(
    dbapi: DBApi,
  ...
  def consultar(
      ...
      empr_id: scala.Long
  ): Future[Iterable[Causado]] = Future[Iterable[Causado]] {
    val base = "default"
    ...
    db.withConnection { implicit connection =>
      ...
      empresa match {
        ...
        case Some(empresa) =>
          val _pParser = str("ID_PERSONA") ~ str("PRIMER_APELLIDO") ~ str(
            "SEGUNDO_APELLIDO"
          ) ~ str("NOMBRE") map { case a ~ b ~ c ~ d => (a, b, c, d) }
          val persona = SQL(
            """SELECT c.ID_PERSONA, p.PRIMER_APELLIDO, p.SEGUNDO_APELLIDO, p.NOMBRE FROM \"col$colocacion\" c INNER JOIN \"gen$persona\" p ON p.ID_IDENTIFICACION = c.ID_IDENTIFICACION and p.ID_PERSONA = c.ID_PERSONA WHERE c.ID_COLOCACION = {id_colocacion}"""
          ).on(
              'id_colocacion -> id_colocacion
            )
            .as(_pParser.single)
          val cauResulset = SQL(
            """SELECT c.FECHA_CORTE, c.VALOR, c.DEUDA, c.ANTICIPADOS, c.CAUSADOS, c.CONTINGENCIAS, c.DIAS, c.TASA, c.FECHA_INTERES FROM \"col$causaciondiaria\" c WHERE c.ID_COLOCACION = {id_colocacion} ORDER BY c.FECHA_CORTE"""
          ).on(
              'id_colocacion -> id_colocacion
            )
            .as(Causado._set *)
          ...
          cauResulset.map { c =>
            ...
            val extResulset = SQL(
              """SELECT e.FECHA_EXTRACTO, e.ID_CBTE_COLOCACION, e.ABONO_CAPITAL, e.ABONO_ANTICIPADO, e.ABONO_CXC, e.ABONO_SERVICIOS, e.INTERES_PAGO_HASTA FROM \"col$extracto\" e WHERE e.ID_COLOCACION = {id_colocacion} and e.FECHA_EXTRACTO BETWEEN {fecha_inicial} and {fecha_final} ORDER BY e.FECHA_EXTRACTO"""
            ).on(
                'id_colocacion -> id_colocacion,
                'fecha_inicial -> fi,
                'fecha_final -> ff
            ...
            extResulset.map { e =>
              val detResultset = SQL(
                """SELECT d.CODIGO_PUC, p.NOMBRE, d.FECHA_INICIAL, d.FECHA_FINAL, d.DIAS_APLICADOS, d.TASA_LIQUIDACION, d.VALOR_DEBITO, d.VALOR_CREDITO FROM \"col$extractodet\" d
  ...
  def extracto(
      ...
      empr_id: scala.Long
  ): Future[Iterable[Extracto]] = Future[Iterable[Extracto]] {
    val base = "default"
    val db = dbapi.database(base)
    db.withConnection { implicit connection =>
      ...
      val extResulset = SQL(
        """SELECT e.FECHA_EXTRACTO, e.ID_CBTE_COLOCACION, e.ABONO_ANTICIPADO, e.ABONO_CXC, e.ABONO_SERVICIOS, e.INTERES_PAGO_HASTA FROM \"col$extracto\" e WHERE e.ID_COLOCACION = {id_colocacion} and e.FECHA_EXTRACTO BETWEEN {fecha_inicial} and {fecha_final} ORDER BY e.FECHA_EXTRACTO"""
      ).on(
          'id_colocacion -> id_colocacion,
          'fecha_inicial -> fi,
          'fecha_final -> ff
    ...
    db.withConnection { implicit connection =>
      ...
      empresa match {
        ...
        case Some(empresa) =>
          ...
          val persona = SQL(
            """SELECT c.ID_PERSONA, p.PRIMER_APELLIDO, p.SEGUNDO_APELLIDO, p.NOMBRE FROM \"col$colocacion\" c WHERE c.ID_COLOCACION = {id_colocacion}"""
          ).on(
              'id_colocacion -> id_colocacion
            )
            .as(_pParser.single)
          val cauResulset = SQL(
            """SELECT c.FECHA_CORTE, c.VALOR, c.DEUDA, c.ANTICIPADOS, c.CAUSADOS, c.CONTINGENCIAS, c.DIAS, c.TASA, c.FECHA_INTERES FROM \"col$causaciondiaria\" c WHERE c.ID_COLOCACION = {id_colocacion}"""
          ).on(
              'id_colocacion -> id_colocacion
            )
            .as(Causado._set *)
          ...
          val extResulset = SQL(
            """SELECT e.FECHA_EXTRACTO, e.ID_CBTE_COLOCACION, e.ABONO_ANTICIPADO, e.ABONO_CXC, e.ABONO_SERVICIOS * FROM \"col$extracto\" e WHERE e.ID_COLOCACION = {id_colocacion}""""
          ).on(
              'id_colocacion -> id_colocacion
            )
            .as(Extracto._set *)
            ...
            rows = {
              ...
              val headerRow = com.norbitltd.spoiwo.model
                ...
                .withCellValues(
                  ...
                  "Días",
                  "Tasa",
                  "Fecha Desde la Cuál se Causa"
                )
              var j = 2
              val rows = cauResulset.map {
                i =>
                  j += 1
                  com.norbitltd.spoiwo.model.Row(
                    StringCell(
                      i.fecha match { case Some(value) => format.format(value) },
                      Some(0),
                      style = Some(
                    ...
                    NumericCell(
                      i.valor match { case Some(value) => value.toDouble },
                      Some(1),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.deuda match { case Some(value) => value.toDouble },
                      Some(2),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.anticipado match { case Some(value) => value.toDouble },
                      Some(3),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.causado match { case Some(value) => value.toDouble },
                      Some(4),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.contingencia match {
                    ...
                    NumericCell(
                      i.dias match { case Some(value) => value.toDouble },
                      Some(6),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.tasa match { case Some(value) => value.toDouble },
                      Some(7),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    StringCell(
                      i.fecha_interes match {
                        case Some(value) => format.format(value)
                      },
                      Some(8),
                      style = Some(
                        CellStyle(dataFormat = CellDataFormat("YYYY-MM-DD"))
              ...
              val rows = extResulset.map {
                i =>
                  ...
                  com.norbitltd.spoiwo.model.Row(
                    ...
                    NumericCell(
                      i.cbte match { case Some(value) => value.toDouble },
                      Some(1),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    ...
                    NumericCell(
                      i.cxc match { case Some(value) => value.toDouble },
                      Some(3),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
                    NumericCell(
                      i.servicio match { case Some(value) => value.toDouble },
                      Some(4),
                      style = Some(CellStyle(dataFormat = CellDataFormat("#0"))),
                      CellStyleInheritance.CellThenRowThenColumnThenSheet
                    ),
