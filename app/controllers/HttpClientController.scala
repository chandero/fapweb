...
import http._
@Singleton
class HttpClientController @Inject()(
  ...
  def gettransactionbyid(f: String, p: String, d: String) = Action.async {
    ...
    var url = conf
      .get[String]("urlFacturacion") + "/GetTransaccionbyIdentificacion/" + f + "/" + p + "/" + d
    println("Cree el cliente http")
    var params = collection.immutable.Map[String, String]()
    http.doGet(url, params).map { response =>
      println("Result: " + response)
...
//            response.flatMap { response =>
  ...
  def setdocumentbydsajson(f: Long) = Action.async { implicit request =>
    println("Estoy en el metodo setdocumentbydsajson")

    http.enviarDSA(f).flatMap { response =>
      println("Result: " + response)
      val contentType = response.headers
        .get("Content-Type")
        .flatMap(_.headOption)
...
//            response.flatMap { response =>
  ...
  def setnotacreditodsabyjson(nc: Long) = Action.async { implicit request =>
    http.enviarNotaCreditoDSA(nc).flatMap { response =>
//            response.flatMap { response =>
      println("Result: " + response)
      val contentType = response.headers
        .get("Content-Type")
