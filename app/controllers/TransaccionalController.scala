...
class TransaccionalController @Inject()(
  ...
  def autenticar(id_identificacion: Int, id_persona: String, contrasena: String): Action[AnyContent] =
    Action.async { request =>
      println("Recibiendo peticion de autenticacion")
      println(id_identificacion)
      println(id_persona)
      println(contrasena)
      transaccionalService.autenticar(id_identificacion, id_persona, contrasena).flatMap { esValido =>
        if (esValido) {
          transaccionalService.buscarPorDocumento(id_identificacion, id_persona).flatMap { cliente =>
            ...
            val client = new UsuarioDto(Some(0),
                                      cliente.get._4,
                                      Some(""),
                                      cliente.get._1,
                                      cliente.get._2,
  ...
  def linkValidator(link: String) = Action.async { request: Request[AnyContent] =>
    transaccionalService.validarEnlace(link).map { result =>
      if (result) {
        Ok(Json.toJson("true"))
      } else {
        NotFound
