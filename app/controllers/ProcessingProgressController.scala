...
class ProcessingProgressController @Inject()(
  ...
  def getProgressStatuses(sessionId: String) = Action {
    ...
    val sourceOfSessionStatuses: Source[String, NotUsed] =
      ProcessingProgressTrackerEvent
        .getSourceOfStatuses(sessionId)
        .getOrElse(
          ProcessingProgressTrackerEvent.createSourceOfStatuses(sessionId)
        )
