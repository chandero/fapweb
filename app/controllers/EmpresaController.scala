...
class EmpresaController @Inject()(
  ...
  def seleccionarEmpresa(empr_id: Long) =
    ...
    val company = new EmpresaDto(empresa.get.empr_id,
                                 ...
                                 empresaToken, uep.get.perf_abreviatura.get)
...
*/
    def guardarEmpresa() =  authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      ...
      val usua_id = Utility.extraerUsuario(request)
      val empresanuevo = new Empresa(Some(0),empresa.empr_descripcion,empresa.empr_sigla,empresa.empr_identificacion,empresa.empr_direccion,empresa.empr_telefono,empresa.empr_estado,usua_id.get,empresa.depa_id,empresa.muni_id)
    ...
    def actualizarEmpresa() =  authenticatedUserAction.async { implicit request: Request[AnyContent] =>
      ...
      println(empresa)
      val usua_id = Utility.extraerUsuario(request)
      val empresanuevo = new Empresa(empresa.empr_id,empresa.empr_descripcion,empresa.empr_sigla,empresa.empr_identificacion,empresa.empr_direccion,empresa.empr_telefono,empresa.empr_estado,usua_id.get,empresa.depa_id,empresa.muni_id)
