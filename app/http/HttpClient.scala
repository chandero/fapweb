...
class HttpClient @Inject()(
  ...
  def testResponse(jsonString: String) = {
    ...
    respBuilder.accumulate(
      new CacheableHttpResponseStatus(
        Uri.create("http://localhost:9005/api/service"),
        202,
        "status text",
        "json"
      )
    ...
    respBuilder.accumulate(
      new CacheableHttpResponseBodyPart(jsonString.getBytes(), true)
    )
  ...
  def setDSADocument(f: Long) = {
    service.enviarDSA(f).map { rootInterface =>
      // var jsonString = Json.stringify(Json.toJson(rootInterface))
      implicit val formats = DefaultFormats
      var jsonString = write(rootInterface)
      jsonString = "{\"Documento\":" + jsonString + "}"
  ...
  def setNotaCredito(nc: Long) = {
    ...
    def setNotaCreditoDSA(nc: Long) = {
    service.enviarNotaCreditoDSA(nc).map { rootInterface =>
      // var jsonString = Json.stringify(Json.toJson(rootInterface))
      implicit val formats = DefaultFormats
      var jsonString = write(rootInterface)
      jsonString = "{\"Documento\":" + jsonString + "}"
      println("json nota crédito dsa: " + jsonString)
      jsonString
    }
  }
  ...
  def setDSADocumentJson(f: Long) = {
    setDSADocument(f).map { jsonString =>
      var url = conf.get[String]("urlFacturacion") + "/SetDocument"
      var params = collection.immutable.Map[String, String]()
      println("Cadena a Enviar:" + jsonString)
      doPost(url, jsonString).map { response =>
  ...
  def setNotaCreditoJson(nc: Long) = {
    ...
    def setNotaCreditoDSAJson(nc: Long) = {
    setNotaCreditoDSA(nc).map { jsonString =>
      var url = conf.get[String]("urlFacturacion") + "/SetDocument"
      var params = collection.immutable.Map[String, String]()
      println("Cadena NC DSA a Enviar:" + jsonString)
      doPost(url, jsonString).map { response =>
        response
      }
    // Se usa solo en pruebas
  ...
  def enviarDSA(f: Long) = {
    setDSADocumentJson(f).flatMap { response =>
      response
    }
  }
  ...
  def enviarNotaCreditoDSA(nc: Long) = {
    setNotaCreditoDSAJson(nc).flatMap { response =>
      response
    }
  }
