...
use crate::api::controller::models::custom_vec_format;

use super::tipo_identificacion::TipoIdentificacion;
use super::tipo_persona::TipoPersona;
use super::tipo_estado_civil::TipoEstadoCivil;
use super::tipo_estado_persona::TipoEstadoPersona;
use super::tipo_relacion::TipoRelacion;
use super::tipo_ciiu::TipoCiiu;
...
impl FromRow for Persona {
    fn try_from(row: Vec<Column>) -> Result<Self, FbError> {
        Ok(Self {
            ...
            fecha_ingreso_empresa: match row[30].clone().to_val() { Ok(v) => v, Err(_) => NaiveDate::from_ymd(0, 0, 0).and_hms(0,0,0) },
            ...
            fecha_registro: match row[52].clone().to_val() { Ok(v) => v, Err(_) => NaiveDate::from_ymd(0, 0, 0).and_hms(0,0,0) },
            ...
            fecha_actualizacion: match row[63].clone().to_val() { Ok(v) => v, Err(_) => NaiveDate::from_ymd(0, 0, 0).and_hms(0,0,0) },
