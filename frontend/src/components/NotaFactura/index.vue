<template>
  <el-container>
    <el-header>
      <h1>Nota {{tipo === 'S' ? 'Crédito DSA':'Factura Electrónica'}}</h1>
    </el-header>
    <el-main>
      <el-form :inline="true" :model="factura">
        <el-form-item :label="tipo === 'S' ? 'DSA Número':'Factura Número'" >
          <el-input v-model="factura.fact_numero" @change="factura.fact_numero=parseInt($event)" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="tipo === 'S' ? getDSAPorNumero(): getFacturaPorNumero()" />
        </el-form-item>
      </el-form>
      <el-row :gutter="4">
      ...
      <el-row>
        <el-col>
          <el-form-item label="Seleccione Débito o Crédito">
            <el-radio v-if="tipo === 'D'" v-model="nota.fact_nota_tipo" label="D">Nota Débito</el-radio>
            <el-radio v-if="tipo === 'C'" v-model="nota.fact_nota_tipo" label="C">Nota Crédito</el-radio>
            <el-radio v-if="tipo === 'S'" v-model="nota.fact_nota_tipo" label="S">Nota Crédito DSA</el-radio>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col>
          <el-button :disabled="isDisabled" type="primary" size="mini" icon="el-icon-plus" title="Agregar Item" @click="nuevoItem">Agregar Detalle</el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
      ...
      <el-row>
        <el-col>
          <el-button :disabled="nota.items.length < 1 || isDisabled" type="primary" icon="el-icon-files" @click="crearNota">Crear y Enviar Nota</el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col>
            <el-input readonly type="textarea" :autosize="{ minRows: 3, maxRows: 10}" v-model="respuestaEnvio" placeholder="Respuesta del Envío"></el-input>
        </el-col>
      </el-row>
      </el-form>
        </el-col>
...
import { getFactura, getDSA, getFacturaProveedor, getDSAProveedor, crearNotaDebito, crearNotaCredito, crearNotaCreditoDSA, enviarNotaDebito, enviarNotaCredito, enviarNotaCreditoDSA } from '@/api/factura'
  ...
  methods: {
    crearNota () {
      this.$confirm('Seguro de Crear y Enviar la Nueva Nota?', 'Atención', {
        ...
        this.nota.fact_numero = parseInt(this.factura.fact_numero)
        if (this.nota.fact_nota_tipo === 'D') {
          crearNotaDebito(this.nota).then(response => {
            if (response.status === 200) {
              this.isDisabled = true
              if (response.data > 0) {
                enviarNotaDebito(response.data).then(response => {
                  this.respuestaEnvio = JSON.stringify(response.data)
                })
              } else {
                this.$alert('No se pudo crear la Nota Débito', 'Error', {
        ...
        } else if (this.nota.fact_nota_tipo === 'C')
        {
          crearNotaCredito(this.nota).then(response =>{
            if (response.status === 200) {
              this.isDisabled = true
              if (response.data > 0) {
                enviarNotaCredito(response.data).then(response => {
                  this.respuestaEnvio = JSON.stringify(response.data)
                })
              } else {
                this.$alert('No se pudo crear la Nota Crédito', 'Error', {
                  confirmButtonText: 'Ok',
                })
              }
            }
          })
        } else if (this.nota.fact_nota_tipo === 'S')
        {
          crearNotaCreditoDSA(this.nota).then(response =>{
            if (response.status === 200) {
              this.isDisabled = true
              if (response.data > 0) {
                enviarNotaCreditoDSA(response.data).then(response => {
                  this.respuestaEnvio = JSON.stringify(response.data)
                })
              } else {
                this.$alert('No se pudo crear la Nota Crédito DSA', 'Error', {
                  confirmButtonText: 'Ok',
                })
              }
            }
    ...
    getDSAPorNumero () {
      getDSA(this.factura.fact_numero).then(response => {
        this.factura = response.data
        this.notfound = false
        this.getCufe()
      }).catch((e) => {
    ...
    getCufe () {
      ...
      if (this.factura.fact_nota_tipo != 'S') {
      ...
      } else if (this.factura.fact_nota_tipo === 'S') {
        getDSAProveedor(this.factura.fact_numero, periodo).then(response =>{
          this.nota.fact_cufe = response.data.GetTransaccionbyIdentificacionResult.CodigoTransaccion
          if (this.nota.fact_cufe) {
            this.factura_pdf = 'data:application/pdf;base64,' + response.data.GetTransaccionbyIdentificacionResult.PDFBase64
          }
