<template>
  <el-container class="libromayor-container">
    <el-main>
      <el-container>
        <el-header class="libromayor_header">{{ $t('librocajadiario.tabletitle') }}
        </el-header>
      </el-container>
      <el-container>
        <el-header>
...
<script>
import { conLibroCajaDiario, verLibroCajaDiario } from '@/api/informe'
...
export default {
  ...
  methods: {
    ...
    consultar() {
      conLibroCajaDiario(this.anho).then(response => {
        this.tableData = response.data
      })
    },
    handleView(index, row) {
      verLibroCajaDiario(row.lire_anho, row.lire_periodo, row.lire_consecutivo)
    }
  }
}
</script>
