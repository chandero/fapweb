<template >
    <el-container>
      <el-header>
        <el-row>
          <el-col :span="24">
            <h1>Informe de Recaudo Vía Web Pendiente por Aplicar</h1>
          </el-col>
        </el-row>
      </el-header>
      <el-main>
          ...
          <el-col :span="24">
            <el-table v-loading="loading" :data="tableData" stripe max-height="400px" style="font-size:12px;">
              ...
              <el-table-column sortable label="Documento" prop="id_persona" width="130">
                <template slot-scope="scope">
                  <span style="margin-left: 10px">{{ scope.row._6 }}</span>
                </template>
              </el-table-column>
              <el-table-column sortable label="Nombre" prop="nombre" width="300">
                <template slot-scope="scope">
                  <span style="margin-left: 10px">{{ scope.row._7 | fm_truncate(50) }}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column sortable label="Valor" prop="cuenta" width="300">
                <template slot-scope="scope">
                  <span style="margin-left: 10px">${{ scope.row._8 | currency_2 }}</span>
                </template>
              </el-table-column>
            </el-table>
          </el-col>
  ...
  import { getLiquidacionPendienteWeb, getLiquidacionPendienteWebXlsx } from '@/api/informe'
  ...
  export default {
    ...
    methods: {
      buscar() {
        this.loading = true
        getLiquidacionPendienteWeb(this.form.fecha_inicial.getTime(), this.form.fecha_final.getTime()).then(response => {
          this.loading = false
      ...
      handleExcel() {
        this.loading = true
        getLiquidacionPendienteWebXlsx(this.form.fecha_inicial.getTime(), this.form.fecha_final.getTime()).then(response => {
          this.loading = false
